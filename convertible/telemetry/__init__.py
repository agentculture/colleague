...
Convertible's "dashboard" (the JSON result artifact + step trace) is per-run and
...
1. **Zero runtime deps** (``tests/test_zero_deps.py``). The OpenTelemetry SDK is
   an *optional extra* (``pip install convertible-cli[otel]``); this module —
   on the import path of :mod:`convertible.loop` — is **stdlib only**. The SDK
   ...
   SDK-backed implementation lives in :mod:`convertible.telemetry._otel`.
...
from convertible.config import _pick, _str
...
_DEFAULT_SERVICE_NAME = "convertible"
...
class TelemetryConfig:
    ...
    Resolution precedence matches :class:`~convertible.config.EngineConfig`:
    explicit value > ``CONVERTIBLE_OTEL_*`` env > standard ``OTEL_*`` env >
    default. Off by default; the standard ``OTEL_SDK_DISABLED=true`` forces it
    off regardless of the other knobs.
    """
    ...
    def resolve(
        ...
        *,
        ...
        enabled_val = _as_bool(
            _pick(_str(enabled), "CONVERTIBLE_OTEL_ENABLED", default="false"), False
        )
        # Standard OTel kill-switch wins: an operator-set OTEL_SDK_DISABLED=true
        # disables convertible telemetry even if CONVERTIBLE_OTEL_ENABLED=1.
        ...
        return cls(
            ...
            metrics_enabled=_as_bool(
                _pick(_str(metrics_enabled), "CONVERTIBLE_OTEL_METRICS_ENABLED", default="true"),
                True,
            ),
        )
...
class Telemetry:
    ...
    The SDK-backed :class:`convertible.telemetry._otel._OtelTelemetry` overrides
...
def load_telemetry(config: TelemetryConfig | None = None, **test_seams: object) -> Telemetry:
    ...
    tests inject in-memory exporters (see :mod:`convertible.telemetry._otel`).
    """
    ...
    try:
        ...
        if not _warned_missing:
            ...
            print(
                "telemetry: requested (CONVERTIBLE_OTEL_ENABLED) but the [otel] extra is "
