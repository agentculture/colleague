...
returns its :class:`~convertible.contract.SubResult`.
...
child engine by name through :func:`convertible.registry.load` and inherits the
parent's :class:`~convertible.config.EngineConfig` with only the model overridden
(``dataclasses.replace``). No engine's own code is touched — selecting a
different model is a config-level switch, exactly the contract Convertible
promises.
"""
...
from convertible.contract import SubResult, Task
...
def run_subagent(
    ...
    *,
    ...
    :data:`~convertible.config.MAX_SUBAGENT_DEPTH` is refused before its drive
    starts, guaranteeing termination.

    The child engine is ``engine or parent_engine``, resolved through
    :func:`convertible.registry.load`. The child config inherits the parent's
