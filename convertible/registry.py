...
An engine becomes available to Convertible by advertising itself under the
``convertible.engines`` entry-point group. The two bundled engines do this in
...
Convertible core (honesty condition h4).

This is the "garage": ``convertible wheels list`` reads :func:`catalog`, and
``convertible drive --engine <name>`` resolves the choice through :func:`load`.
"""
...
ENTRY_POINT_GROUP = "convertible.engines"
...
def _engine_entry_points() -> list[EntryPoint]:
    """All entry points in the convertible.engines group (monkeypatch seam for tests)."""
