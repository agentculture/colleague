"""Convertible TUI state — canonical CockpitState and its nested dataclasses.
...
:mod:`convertible.contract`.  ``json.dumps(state.to_dict())`` must always succeed.
...
@dataclass
class Drive:
    """Snapshot of the currently-running (or last-run) convertible drive.

    Fields
    ------
    task_id:
        The task identifier from :class:`convertible.contract.Task`.
    engine:
