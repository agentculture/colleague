"""Convertible TUI — pure cross-mirror differ (the ``diagnose`` engine).
...
``dataclasses`` / ``typing`` and its sibling ``convertible.tui`` modules.  It
...
#: :func:`convertible.tui.selectors.resolve` (they have no node in the tree).
#: Kept in sync with the standing entries added by
#: :func:`convertible.tui.selectors.selectors`.
...
def _popup_title(popup: dict[str, Any]) -> str:
    ...
    Kept in sync with :mod:`convertible.tui.widgets.popup_layer`: the widget
...
def _is_addressable(taui: dict[str, Any], selector: str) -> bool:
    ...
    :func:`convertible.tui.selectors.resolve`, or it is one of the standing
    ...
    :func:`convertible.tui.selectors.selectors`: that function harvests
