...
<!-- learned-from: claude; source: .claude/skills/challenge/SKILL.md; scripts: -; adapt: claude->colleague -->
...
In a colleague work item, `capture`, `interrogate`, `park`, and `converge`
are on the curated `devague` loop tool allow-list and may be invoked through
the `devague` tool; `scope`, `question`, `review`, `confirm`, `reject`,
`export`, and `plan risk` are **outside** that allow-list, so they run as the
`devague` CLI — directly by the operator in a session, or by a work-loop agent
through `run_command` where the operator's approval gate allows. `confirm`
and `reject` are user-only decisions either way.
