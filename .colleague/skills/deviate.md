...
<!-- learned-from: claude; source: .claude/skills/deviate/SKILL.md; scripts: -; adapt: claude->colleague -->
...
The `devague deviate` moves are all **outside** the curated colleague
`devague` loop tool allow-list (which covers frame moves only), so this leg
runs through the `devague` CLI — the same agent-agnostic commands for any
agent or operator (if `devague` isn't on `PATH`: `uv tool install devague`).
In a session the operator runs the commands directly; in a colleague work
item a task agent reaches them through `run_command` only where the
operator's approval gate allows. `--confirm` and `--reject` are user-only
