...
devague`). In a colleague work item, the frame moves on the curated `devague`
loop tool allow-list (`new`, `capture`, `interrogate`, `park`, `converge`,
`status`, `show`) may be invoked through the `devague` tool; the `scope` and
`question` moves are **outside** that allow-list, so they run as the `devague`
CLI — directly by the operator in a session, or by a work-loop agent through
`run_command` where the operator's approval gate allows.
