...
<!-- learned-from: claude; source: .claude/skills/summarize-delivery/SKILL.md; scripts: -; adapt: claude->colleague -->
...
commands for any agent or operator. In a colleague work item, `show` and
`status` are on the curated `devague` loop tool allow-list (available through
the `devague` tool); `summary`, `deviate --list`, `plan show`, `plan waves`,
and `scope --list` are CLI-only — the operator runs them directly in a
session, and a work-loop agent reaches them through `run_command` only where
the approval gate allows. This skill never mutates devague state, so none of
its moves needs a user-only decision. The artifact itself is written with the
agent's normal file tools.
