...
def test_config_show_prints_ceilings_and_window(
    ...
    rendered = _config_show(str(tmp_path))
    text = rendered._text
    assert (
...
def test_kill_switch_line(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("COLLEAGUE_LOBES_URL", raising=False)
