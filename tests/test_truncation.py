...
def test_truncate_output_keeps_head_and_tail(tmp_path: Path):
    ...
    result = truncation.truncate_output(text, max_chars=300, max_lines=10, spill_dir=spill_dir)
...
def test_spilled_file_named_by_content_hash(tmp_path: Path):
    ...
    truncation.truncate_output(text, max_chars=300, max_lines=10, spill_dir=spill_dir)
...
def test_session_cap_is_tracked_across_calls(tmp_path: Path, monkeypatch):
    ...
    first = truncation.truncate_output(text, max_chars=300, max_lines=10, spill_dir=spill_dir)
    ...
    with pytest.warns(RuntimeWarning):
        second = truncation.truncate_output(text, max_chars=300, max_lines=10, spill_dir=spill_dir)
