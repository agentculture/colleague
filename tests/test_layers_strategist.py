"""Tests for the bounded task-local strategist prompt section (plan t5).
...
Acceptance (verbatim from the plan):
    ...
    task-local strategist section; base prompt, AGENTS layers, role prompts,
...
directly with the new optional strategist_section/strategist_seat kwargs.
"""
...
from colleague.layers import (
    STRATEGIST_SEAT_SENSES,
    STRATEGIST_SEAT_WORKER,
    STRATEGIST_SECTION_HEADING,
    STRATEGIST_SECTION_MAX_CHARS,
    StrategistSectionTooLarge,
    compose_role_prompt,
    compose_strategist_section,
    system_prompt_for,
)
...
# --- compose_strategist_section: the pure primitive --------------------------


class TestComposeStrategistSectionPrimitive:
    def test_none_text_renders_nothing(self) -> None:
        assert compose_strategist_section(None, STRATEGIST_SEAT_WORKER) is None

    def test_empty_text_renders_nothing(self) -> None:
        assert compose_strategist_section("", STRATEGIST_SEAT_WORKER) is None

    def test_whitespace_only_text_renders_nothing(self) -> None:
        assert compose_strategist_section("   \n  ", STRATEGIST_SEAT_SENSES) is None

    def test_valid_text_worker_seat_renders_named_section(self) -> None:
        section = compose_strategist_section("Focus on the auth module.", STRATEGIST_SEAT_WORKER)
        assert section is not None
        assert section.startswith(STRATEGIST_SECTION_HEADING)
        assert "Focus on the auth module." in section

    def test_valid_text_senses_seat_renders_named_section(self) -> None:
        section = compose_strategist_section("Relay tersely.", STRATEGIST_SEAT_SENSES)
        assert section is not None
        assert section.startswith(STRATEGIST_SECTION_HEADING)
    ...
    def test_heading_is_the_documented_example(self) -> None:
        ...
        assert STRATEGIST_SECTION_HEADING == "## Strategist (task-local)"

    def test_text_is_stripped(self) -> None:
        section = compose_strategist_section("  padded text  \n", STRATEGIST_SEAT_WORKER)
        assert section == f"{STRATEGIST_SECTION_HEADING}\n\npadded text"

    def test_unknown_seat_raises(self) -> None:
        with pytest.raises(ValueError):
            compose_strategist_section("text", "cortex")
    ...
    def test_unknown_seat_raises_even_with_empty_text(self) -> None:
        ...
        with pytest.raises(ValueError):
            compose_strategist_section(None, "not-a-seat")

    def test_at_cap_boundary_succeeds(self) -> None:
        text = "x" * STRATEGIST_SECTION_MAX_CHARS
        section = compose_strategist_section(text, STRATEGIST_SEAT_WORKER)
        assert section is not None
        assert text in section

    def test_over_cap_raises_too_large_never_truncates(self) -> None:
        text = "x" * (STRATEGIST_SECTION_MAX_CHARS + 1)
        with pytest.raises(StrategistSectionTooLarge):
            compose_strategist_section(text, STRATEGIST_SEAT_WORKER)

    def test_too_large_is_a_value_error_subclass(self) -> None:
        # A caller that only catches ValueError still catches this.
        assert issubclass(StrategistSectionTooLarge, ValueError)


# --- system_prompt_for: strategist section threading -------------------------


class TestSystemPromptForStrategist:
    def test_default_unused_is_byte_identical_to_today(self, tmp_path: Path) -> None:
        ...
        with_none = system_prompt_for(
            repo, _MODEL_X, user_home=home, base=_BASE_PROMPT, strategist_section=None
        )
        assert without_kwarg == with_none

    def test_none_repo_stays_none_when_strategist_absent(self, tmp_path: Path) -> None:
        repo = _repo(tmp_path)
        home = _home(tmp_path)
        assert (
            system_prompt_for(
                repo, _MODEL_X, user_home=home, base=_BASE_PROMPT, strategist_section=""
            )
            is None
        )

    def test_strategist_alone_produces_a_prompt(self, tmp_path: Path) -> None:
        repo = _repo(tmp_path)
        home = _home(tmp_path)
        prompt = system_prompt_for(
            ...
            user_home=home,
            base=_BASE_PROMPT,
            strategist_section="Prioritize tests.",
        )
        assert prompt is not None
        assert prompt == f"{_BASE_PROMPT}\n\n{STRATEGIST_SECTION_HEADING}\n\nPrioritize tests."

    def test_order_base_agents_strategist_skills(self, tmp_path: Path) -> None:
        repo = _repo(tmp_path)
        home = _home(tmp_path)
        ...
        prompt = system_prompt_for(
            ...
            user_home=home,
            base=_BASE_PROMPT,
            strategist_section="STRATEGIST-TEXT",
        )
        assert prompt is not None
        idx_base = prompt.index(_BASE_PROMPT)
        idx_agents = prompt.index("AGENTS-LAYER")
        idx_strategist = prompt.index("STRATEGIST-TEXT")
        idx_skills = prompt.index("Available skills")
        assert idx_base < idx_agents < idx_strategist < idx_skills
    ...
    def test_baseline_vs_configured_prefix_and_suffix_pinned(self, tmp_path: Path) -> None:
        ...
        composed prompt differs in EXACTLY one named task-local strategist
        ...
        strategist_block = f"{STRATEGIST_SECTION_HEADING}\n\nCORTEX-AUTHORED-NOTE"
        ...
        # The only difference is the one named strategist section, inserted
        # between the pinned prefix and the pinned suffix.
        assert baseline == prefix + skills_text
        assert configured == prefix + strategist_block + "\n\n" + skills_text

    def test_oversize_strategist_section_propagates(self, tmp_path: Path) -> None:
        repo = _repo(tmp_path)
        home = _home(tmp_path)
        with pytest.raises(StrategistSectionTooLarge):
            system_prompt_for(
                ...
                user_home=home,
                base=_BASE_PROMPT,
                strategist_section="x" * (STRATEGIST_SECTION_MAX_CHARS + 1),
            )
    ...
    def test_invalid_seat_propagates(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(ValueError):
            system_prompt_for(
                ...
                user_home=home,
                base=_BASE_PROMPT,
                strategist_section="text",
                strategist_seat="cortex",
            )


# --- compose_role_prompt: strategist section threading -----------------------


class TestComposeRolePromptStrategist:
    ...
    def test_default_unused_is_byte_identical_to_today(self, tmp_path: Path) -> None:
        ...
        with_none = compose_role_prompt(
            role, repo, _MODEL_X, base=_BASE_PROMPT, strategist_section=None
        )
        assert without_kwarg == with_none

    def test_order_base_agents_role_strategist_skills(self, tmp_path: Path) -> None:
        repo = _repo(tmp_path)
        _write(repo / "AGENTS.md", "AGENTS-LAYER")
        _write(repo / ".colleague" / "skills" / "alpha.md", "# alpha\nAlpha skill.")
        role = self._role()
        prompt = compose_role_prompt(
            role, repo, _MODEL_X, base=_BASE_PROMPT, strategist_section="STRATEGIST-TEXT"
        )
        assert prompt is not None
        idx_base = prompt.index(_BASE_PROMPT)
        idx_agents = prompt.index("AGENTS-LAYER")
        ...
        idx_strategist = prompt.index("STRATEGIST-TEXT")
        idx_skills = prompt.index("Available skills")
        assert idx_base < idx_agents < idx_role < idx_strategist < idx_skills
    ...
    def test_role_name_string_path_also_threads_strategist(self, tmp_path: Path) -> None:
        """compose_role_prompt(role_name_str, ...) — the unknown-role fallback
        path — also threads the strategist kwargs through to system_prompt_for."""
        repo = _repo(tmp_path)
        prompt = compose_role_prompt(
            "no-such-role",
            repo,
        ...
        assert prompt is not None
        assert "STRATEGIST-TEXT" in prompt
...
def test_seat_constants_match_lattice_seat_names() -> None:
    ...
    # WORKER_PROMPT_STRATEGIST / SENSES_PROMPT_STRATEGIST targets name.
    assert STRATEGIST_SEAT_WORKER == "worker"
    assert STRATEGIST_SEAT_SENSES == "senses"
