...
def test_mock_drives_and_writes_marker(tmp_path: Path) -> None:
    ...
    result = MockEngine().work(task, EngineConfig.resolve())
...
def test_mock_is_deterministic(tmp_path: Path) -> None:
    ...
    first = MockEngine().work(task, cfg).to_dict()
    second = MockEngine().work(task, cfg).to_dict()

    # Wall-clock fields in WorkStats (started_at, duration_seconds) are
...
def test_mock_never_touches_the_network(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = MockEngine().work(Task.new(str(tmp_path), "offline"), EngineConfig.resolve())
    assert result.status == OK
