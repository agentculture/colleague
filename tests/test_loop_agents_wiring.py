...
def test_armed_run_records_every_invocation_and_folds_the_block(armed) -> None:
    ...
    assert inv["purpose"] == "thinker_coder" and inv["resolved_model"] == cfg.model
    assert inv["token_estimate"] > 0 and inv["token_estimate_source"] in ("chars", "tokenize")
    assert block["ledger_path"] == str(ledger_path(repo, task.id)) and block["ledger_digest"]
    # the ledger at the OPERATOR repo carries the immutable request first
    events = read_ledger(Path(block["ledger_path"])).events
    assert events[0].kind == "operator_request" and events[0].data["text"] == task.instruction
    kinds = [e.kind for e in events]
    assert kinds.count("invocation") == 3 and "constraint" in kinds and "acceptance" in kinds
    # the static guidance + nucleus rode the system prompt ONCE (cache-friendly)
    assert "associate" in seen_system[0] and seen_system[0] == seen_system[1]
...
def test_operator_input_outranks_and_lands_on_the_ledger(armed) -> None:
    ...
    assert ops and ops[-1].data["text"].startswith("actually") and ops[-1].seq > events[0].seq
...
def test_worker_purpose_is_narrowed_on_both_halves(tmp_path: Path) -> None:
    ...
    assert "write_file" not in offered and "edit_file" not in offered and "read_file" in offered
    ...
    # thinker_coder keeps the full surface: no narrowing
    setattr(cfg, "agents_profile", "thinker_coder")
    assert loop.resolve_role(cfg, str(repo)) is None
...
def test_aborted_run_still_folds_the_block(armed) -> None:
    ...
    with pytest.raises(loop.WorkAborted) as excinfo:
        loop.run(boom, task, max_steps=2, context=ContextControls.from_config(cfg))
    partial = excinfo.value.result
    assert partial.agents is not None and partial.agents["ledger_path"]
