...
def test_overview_identity_surfaces_work_model_consistent_with_whoami(
    ...
    assert f"work engine: {ident['work_engine']}" in out
    assert f"work model: {format_work_model(ident['work_model'])}" in out
