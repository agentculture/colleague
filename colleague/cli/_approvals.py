...
Both ``approve`` verbs write the same ``<repo>/.colleague/approvals.json``
ledger. Centralizing the write here keeps the two command modules free of a
duplicated merge-and-write helper and a repeated ``".colleague"`` literal — the
path is built once, from :data:`colleague.configdir.CONFIG_DIR_NAME`, and
confined to the repo root.

Reading approvals for *display* deliberately goes through
:func:`colleague.policy.load_policy` (repo-over-user + per-model overlay), so
...
from colleague.policy import POLICY_FILENAME
...
def write_approval(repo: Path, category: str, name: str, checksum: str) -> None:
    ...
    The ledger lives at the fixed sub-path ``.colleague/approvals.json`` under
    *repo*. The target is resolved and **confined to the resolved repo root** —
    the same defense :meth:`colleague.tools.ToolExecutor._safe_path` applies —
    so the operator-supplied ``--repo`` can never steer the write outside the
    repository tree (``..`` segments, a symlinked root). Creates ``.colleague/``
