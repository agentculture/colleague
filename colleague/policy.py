...
``.colleague/approvals.json`` (repo-over-user via :mod:`colleague.configdir`,
plus a per-model overlay ``.colleague/<sanitize_model(model)>/approvals.json``
...
with colleague's zero-runtime-deps convention.
"""
...
from colleague.configdir import resolve_file
from colleague.layers import sanitize_model

#: The approval-gate config filename, resolved through ``.colleague/``.
...
def _parse_policy_file(path: Path | None) -> dict[str, dict]:
    ...
    :func:`colleague.hooks._parse_hooks_file`. Within a valid object, only
...
def load_policy(
    ...
    *,
    ...
    Resolution mirrors :func:`colleague.hooks.load_hooks`:

    1. The base ``approvals.json`` is resolved via
       :func:`colleague.configdir.resolve_file` (repo-over-user precedence).
    2. When *model* is given, the per-model overlay
       ``.colleague/<sanitize_model(model)>/approvals.json`` is resolved
       through the **same** configdir machinery. The overlay path is built by
       **exact construction** via :func:`colleague.layers.sanitize_model` —
       sibling ``.colleague/*/`` directories are never globbed, so model X can
