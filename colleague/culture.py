...
the child via :func:`colleague.identity.identity_env` so the CLI inherits
``COLLEAGUE_IDENTITY`` (and the legacy ``CONVERTIBLE_IDENTITY`` alias), and runs
with ``cwd`` pinned at the repo root so a CLI
