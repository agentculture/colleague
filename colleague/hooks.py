...
Loads ``.colleague/hooks.json``, matches hook entries to lifecycle events
...
Config format (`.colleague/hooks.json`)::
...
class HookConfig:
    ...
    def all_entries(self) -> list[HookEntry]:
        ...
        Public accessor for read-only enumeration (e.g. ``colleague hooks
...
def load_hooks(
    ...
    *,
    ...
    """Load ``.colleague/hooks.json`` for *repo_path*, optional per-model overlay.

    Resolves the base file using :func:`colleague.configdir.resolve_file` (repo
    over user).  When *model* is given, additionally resolves the per-model
    overlay ``.colleague/<sanitize_model(model)>/hooks.json`` via the **same**
...
def hook_approval_verdict(command: str, policy: Any, repo_root: Path | str) -> Any:
    ...
    **first denial wins** and is returned immediately.  When no token resolves to
    ...
    Args:
        ...
        policy:  A :class:`~colleague.policy.Policy` instance.
        repo_root: The root of the repo being driven.

    Returns:
        A :class:`~colleague.policy.Verdict` — ``allowed=True`` when all
    ...
    from colleague.policy import Verdict  # noqa: PLC0415
