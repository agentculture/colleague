...
- Evidence trail (operator, 2026-08-30): a survey child's digest must hand the caller a means to VERIFY FAST — every claim cites path:line-range plus a short verbatim excerpt, and the digest lists the commands/searches it ran — so the parent re-validates by ranged reads of the cited lines instead of re-reading whole modules (the re-do pattern seen in 5 of 6 delegating runs). Applies to `code_survey`/`web_survey` (and review/validate/plan digests by extension).
- Nemotron associate = a fast 128K SURVEY / MEMORY layer with explicit code-semantic escalation (#461, operator 2026-08-30): strong/default fits — `web_survey`, web/document reconnaissance, document reading, conversation/history survey, memory extraction/building, file/repo inventory, grep/search/symbol discovery, evidence bundles; `code_survey` is LIMITED to evidence discovery (paths, symbols, definitions/references/call sites, bounded excerpts, commands run); semantic code understanding, tracing behaviour, bug diagnosis, design and code changes ESCALATE to cortex/coder. 128K is a CEILING, not a target occupancy — colleague clamps the associate window on its side (the advert over-claims 1,048,576). Measurement of prefill/context use, latency, evidence quality and the cortex redo-rate ties back to #459.
- Associate (Nemotron) usage contract, measured by the operator (2026-08-30): address the lane as model 'associate' (never the raw checkpoint id — it 404s `role_infeasible` on a proxying box); served window 128,000; /v1/\* needs the bearer key. TWO profiles: DEPTH PASS (research, survey, code location) = temperature 0.6, `top_p` 0.95, `chat_template_kwargs`.`enable_thinking` TRUE, and `max_tokens` OMITTED (a low cap is the failure mode: `max_tokens` 4096 with thinking on returned empty content with HTTP 200 + `finish_reason` length on 8/12 tasks; if a cap is unavoidable use >= 8192 — the depth arm used up to 9,730 output tokens on a 23K prompt). FAST TRIAGE (classification, routing, a bounded answer) = temperature 0.6, `top_p` 0.95, `max_tokens` 2048, thinking OFF — ~4x faster, never truncates, but terse/shallower (11 vs 25 durable facts, 3/5 vs 5/5 relevant files, missed the decisive line depth quoted; ignores output-format instructions more often). Rules: never ask this lane for corpus-wide facts (counts, inventories, exhaustive lists — confident fabrications; compute them and pass them in); treat every returned file path as unverified and re-resolve locally (basenames reliable, full paths wrong ~40%).
- Correction to c49 (operator, 2026-08-30): the FAST-TRIAGE profile uses temperature 0.2 (not 0.6); and for colleague's use case every associate lane needs reasoning, so DEPTH (thinking ON, temperature 0.6, `top_p` 0.95, no small `max_tokens`) is the profile for ALL associate sub-seats — TRIAGE is available only as an explicit operator override, never a colleague default.
- Correction to c49 (operator-measured, #461 second comment, 2026-08-30): the fabrication/path-error findings were thinking-OFF numbers. Thinking ON: exact path 75 % (basename 92 %, definition line 100 %), exact counts on trap-free corpora to 40 items / 42,826 tokens, degrading on large ambiguous corpora (3/3 at 46k to 1/3 at 91k+); thinking OFF cannot aggregate at any size. The code-semantic escalation is conditional on the seat's reasoning budget, except design/implement a change (unconditional). Measured rule: a 64K cap does not make counting correct at 91K, it makes 91K requests impossible — reduce the working set before counting is caller-side policy, so colleague keeps the served 128K clamp and owns the reduction. Guide §0b/§4 carry the figures.
