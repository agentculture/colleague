...
| Artifact record | `TaskResult.memory` | Omit-when-None `{query, recalled, injected_chars, lesson_recorded}` plus the retrieval-precision fields below, the rung-2 distill counters when armed, and `split_recorded: true` ONLY when a #416 split-next-time record was written (absent otherwise — the ordinary key set is unchanged) — h7: a misleading recall is diagnosable from the artifact, never silent. |
