...
| `COLLEAGUE_<PURPOSE>_CONTEXT_BUDGET` | unset (opt-in) | Caps ONE purpose child's context window (#458 re-scoped; e.g. `COLLEAGUE_CODE_SURVEY_CONTEXT_BUDGET=65536`); unset/invalid = the child inherits the parent's budget, byte-identical. Row-64b experiment lever, not a default. | `colleague/efforttables.py` |
...
| `COLLEAGUE_HIRE` | unset/`0` | Not a ported mechanism — the delegation-follow-ups hire arming flag (spec c17/h8), listed here because `colleague/prompttext.py` (a marker-carrying module) reads it as the env fallback for the armed-only `HIRE` prompt sentence (`default_system(hire=None)`); the resolved flag itself lives on `EngineConfig.hire` (`colleague/config.py`, env > config.json `hire` > OFF). Unarmed = byte-identical: no hire schemas offered, no sentence rendered. | `colleague/prompttext.py` (`colleague/config.py`, `colleague/hire_schemas.py`) |
