...
> After a work item edits the tree, capture the change as a branch + commit, and —
> when allowed — push and open a pull request.

The handoff (`colleague/handoff.py`) is the last leg of a work item: it turns the
...
ever pushing — so offline and CI work items never touch the network.

## Failure degrades, never raises

A push or PR-creation failure degrades to the same local-commit outcome rather
than aborting the work item. The result note stays honest about what actually
...
Handoff is driven by `colleague work` / `colleague session`; there is no
standalone handoff verb.

```bash
colleague work "..." --repo . --engine vllm-openai            # branch + PR (if gated on)
colleague work "..." --repo . --engine mock --no-pr           # local commit only
colleague work "..." --repo . --engine mock --base develop    # PR against develop
```
...
- [work-and-loop.md](work-and-loop.md) — `branch` / `pr_url` on `TaskResult`.
