...
| 32 | #378 correction-diff ablation | `colleague/correction.py`, `colleague/feedback.py` | ⏳ | RECIPE RECORDED (executable, both arms scripted — same task list, COLLEAGUE_MEMORY=0 as the OFF arm, correction-diff lines per task as the metric, equal-or-worse recorded as falsifying); needs N sequential real tasks — pending a future session | — |
