...
| 37 | #411 model-bound agents — live armed run (agents mode, worker dormant → recorded cortex fallback) | `colleague/agents/*`, `colleague/loop.py` seams | ⏳ | 2026-08-21 (LIVE, pending — the armed `colleague work` on the Spark rig is running; this row records task id, `status`, invocation count / attribution, fallbacks, manifest ratio vs 1,048,576, and whether the `associate` subagent (absent role → cortex fallback) spawned, once it lands) | #411 |
