...
| 32 | #378 correction-diff ablation | `colleague/correction.py`, `colleague/feedback.py` | ⏳ | RECIPE RECORDED (executable, both arms scripted — same task list, COLLEAGUE_MEMORY=0 as the OFF arm, correction-diff lines per task as the metric, equal-or-worse recorded as falsifying); **2026-08-07 (#387 session): the benchmark now EXISTS** — the Transformer game template (`~/git/transformer`, plan of 8 browser-verifiable tasks + FROZEN correction rules, commits `a6bd8f7`/`c30ca2c`) is the committed task list; arms HELD on the webglass M0–M2 usable-bar (webglass-cli#9 — all seven consumer items committed to that slice; status posted on the thread). The challenge pass hardened the design: per-task PRs (the `pr_url`+`tip_sha` metric precondition), SEPARATE arm repos/stores (`memory.remember` is loop-ungated), ON arm first (c32) | — |
