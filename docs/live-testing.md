...
| 64 | delegation-follow-ups-a7-p3-hire (deviation d4, 2026-08-30): **arm R-nemotron — delegation EXPLICITLY REQUESTED by the brief, children on the associate (Nemotron 3.5 Lightning via the Orin proxy)** | `docs/live-testing/briefs/arm-large-surface-requested.md`, `colleague/associate_config.py`, `scripts/compare_arms.py` | ✅ | PRE-REGISTERED 2026-08-30, BEFORE any run, queued behind row 63. Everything not stated here is as rows 59-61 (fixture, rig, comparator freeze, scope line, the six cells, the h16 artifact-id rule) and as `docs/live-testing/briefs/arm-large-surface-requested.md` (the pass bar). **Brief:** that file's brief, verbatim — item 2 REQUESTS delegation (four `code_survey` calls, one per three-module group, verify only the cited lines afterwards). **No overlay; digest clause:** every artifact's `prompt_digest` must equal the no-overlay digest `b7491476a61238a4…` (the task text is not in the system prompt; verified on `9ec6760` that neither the add knob nor the armed associate changes it). **Extra cells:** compliance (>= 4 `code_survey` before the first module-body `read_file`); child wall per call from the parent step timestamps + `sub_results[].usage` + served model; post-digest reads classified ranged-verify vs full-module-redo; wall vs the pooled in-seat median 392 s (rows 57-61, 9 non-delegating runs; delegating runs' median 707 s). **Pooled reading these rows extend (d4):** A5+A6 (2026-08-29/30 a.m.) delegated 5/6; A7+P2-0+P3 (2026-08-30) 1/9 — same brief, fixture and served model, digests as pre-registered — the rate moved across a day with no lever the matrix credits, and in 5 of the 6 delegating runs the parent re-read the four duplicate-pair modules in full after the digests. n=3. **Arming:** `COLLEAGUE_ASSOCIATE_MODEL=lobes` (row 62's precondition: the lane answers from `nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4`; `config show` pasted per run); every child must record that served model — a cortex child is a recorded fallback and VOIDS the run. **The question this arm can answer and no earlier arm could:** does delegation speed the result up when it happens on a fast seat — the pass bar is in the brief file (mean wall < 392 s AND ranged post-digest reads); a miss is written as a miss. **The hire arm moves to row 65 (d4).** **Result: RECORDED 2026-08-30 — ONE run; the arm is INCOMPLETE and re-runs on the t22 tip (d5).** Run 1 artifact `23680ccfc1ad` (tip `2e17abe`, 12:55:34–13:12:01, after the operator's Orin restart; an earlier attempt was VOIDED at 3 min for that restart, runner log 12:21). Runs 2 and 3 were SKIPPED on the operator's decision at 13:20 (deviation d5): every associate child died the same way until #460 was fixed. **Validity:** `prompt_digest` `b7491476…`, `offered_tools` 20 purpose-only names; `config show` with the sentinel resolved the associate (served `nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4`, wire `associate`); the gateway probe answered from that model in 0.26 s. **Compliance: yes** — four `code_survey` calls at step 1 before any module read. **Children: 0 of 4 ran to completion — all four calls REFUSED** with `HTTP 404 role_infeasible` on the served id, each preceded in the run log by `model pin refresh (call) — associate model 'associate' (pinned via call-time-role-alias-rejected) … refreshed to 'nvidia/NVIDIA-Nemotron-…'`: the child read 13 steps of modules, crossed the Orin's SERVED window (128,000 — `/tokenize` through the alias reports it; the advert claims 1,048,576, from which colleague derived a 768,000-token child budget), got a context-length 400, and `associate.retry_role_alias` turned it into the 404 (root cause on #460; deployment side lobes-cli#234). **Parent behaviour after the refusals: 32 steps, full `read_file` of all TWELVE modules (a–l, several twice), 0 ranged reads** — it did the whole survey itself. 14 turns, 977 s, reasoning 11,660, `batches_run` 4 / `calls_parallelised` 24; task `ok`, `mod_g` edited, zero `def` lines changed; markup 0. **Verdict: cannot be measured on this tip** — no associate child produced a digest, so neither the speed bar (mean wall < 392 s) nor the ranged-read bar applies; recorded as such, not as a miss of the seat. **Re-run precondition (pre-registered now):** the tip carrying t22 (served-window clamp + alias guard) AND t23 (the operator's measured profile: temperature 0.6, top_p 0.95, `enable_thinking: true`, no `max_tokens`), `config show` pasting the profile and the served window, the Orin's serving parameters pasted beside them, n=3, and the operator's explicit go (associate testing is stopped until then). Refs: #460, lobes-cli#234, #461, plan t22/t23, `docs/features/associate-validation.md`. **RE-RUN RESULT: RECORDED 2026-08-30 — n=3 on the t22/t23 tip `d0ff8c0`, operator go at ~19:00, associate armed (`COLLEAGUE_ASSOCIATE_MODEL=lobes`), Orin restarted and retuned by the operator (Orin-side serving parameters, read off the deployment 2026-08-30 ~22:15 via ssh: `vllm/vllm-openai:v0.27.1` (container `model-gear-vllm-associate`, compose `~/.lobes/docker-compose.yml` + `.shape` + `.gpu`, restart unless-stopped, behind the `lobes-gateway` container on :8000): `vllm serve nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4 --served-model-name=<same> --quantization=modelopt --kv-cache-dtype=bfloat16 --max-model-len=128000 --gpu-memory-utilization=0.80 --mamba-backend=flashinfer --mamba-ssm-cache-dtype=float16 --enable-mamba-cache-stochastic-rounding --mamba-cache-philox-rounds=5 --mamba-cache-mode=align --enable-prefix-caching --max-num-batched-tokens=16384 --speculative-config={method: dspark, model: …-DSpark, num_speculative_tokens: 5, kv_cache_dtype: bfloat16} --enable-auto-tool-choice --tool-call-parser=qwen3_coder --reasoning-parser=nemotron_v3 --trust-remote-code`; model generation_config defaults (apply only when the client omits sampling): do_sample, temperature 1.0, top_p 0.95 — colleague's depth profile always sends its own. Gateway-visible facts per run: root `/tokenize` `max_model_len` 128,000, `config show` resolving `associate` → `nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4` with profile `depth` (temperature 0.6, top_p 0.95, thinking on, `max_tokens` omitted) — both captured beside each artifact as `Rn2-<n>.config.json` / `.tokenize.json`).** Artifacts `d5c57e32a644` (19:55:05–20:19:49), `a3e4b1726bf9` (20:19:49–20:54:46), `a56de2c569e6` (20:54:46–21:22:46); comparator unchanged (sha `f7e25fdc…`). **Validity: 3/3** — `prompt_digest` `b7491476…`, `offered_tools` the 20 purpose-only names, no raw name; every child `model: associate` (14 of 14 — zero cortex fallbacks, zero refusals, zero pin-refreshes: the #460 fix holds live). **Compliance: 3/3** — four `code_survey` calls at parent steps 1–4 in one batch, first module-body read at step 5–7; run 3 issued two further `code_survey` calls (six children). **Children (Nemotron, thinking on, #458 cap still unapplied):** run 1 `ok`/`ok`/`incomplete`/`incomplete` at 513k/304k/800k/709k total tokens (2.33M); run 2 `incomplete`/`ok`/`ok`/`ok` at 846k/633k/460k/718k (2.66M); run 3 five `ok` + one `incomplete` at 435k/254k/374k/800k/84k/455k (2.40M) — 10 of 14 `ok`, the incompletes are forced syntheses at the child budget, and EVERY digest was used by the parent. **Post-digest reads — RANGED VERIFY in 3/3 (the first arm to pass this bar):** 7 / 8 / 8 `read_file` calls, ALL with a line range (15–40 lines), plus 4 / 2 / 4 `grep_search`, **0 full-module reads in any run** — the parent's own narration: *"The digests are in. Now let me verify the cited lines before editing"*. **(2) Markup: 0.** **(3) Task success: 3/3 `ok`** (`mod_i`, `mod_k`, `mod_l` edited). **(4)/(5) vs row 60 (`compare_arms.py --bar-wall 1.2 --bar-turns 1.0`, P2-0 baseline `8e49d9d936cd,6d327403fd15,0efe5cf792ca`): mean turns 11.00 vs 11.67 → 0.943 (pass); mean wall 1,742.9 s vs 1,071.65 s → 1.626 (MISS).** Per-run wall 1,474 / 2,085 / 1,669 s. vs the pre-registered speed bar (< 392 s, the in-seat median): **MISS** at 3.8–5.3×. vs row 63 (cortex children, mean 3,447.5 s): **2.0× faster** (0.506). **(6) Reasoning chars:** 5,501 / 10,307 / 10,882 (the parent reasons little; the cost is in the children: 2.3–2.7M tokens per run, about the same as row 63's 2.0–2.9M — the Orin's ~80 tok/s is where the time went). `counts`: `batches_run` 3/2/3, `calls_parallelised` 9/8/12. **Reading:** on a fast, thinking-on associate the two things row 63 could not show both happen — delegation completes without refusal and the parent TRUSTS the digests (ranged verification, no redo) — but delegation is still slower than doing the survey in-seat on this brief, and the whole gap is child over-reading (thinking-on children read to their budget; #458's unapplied 12-step cap is the lever to test next, pre-registered as the row-64b condition). The speed bar is written as a miss; the ranged-read bar as a pass. Not a cortex-`low` vs associate head-to-head: row 63's children ran at the scout rung `off`; that comparison is #459's decision rule. Refs: #458, #459, #460, #461, PR #464. | #456 |
| 64b | delegation-follow-ups follow-on (#458 re-scoped, 2026-08-30): **arm R-nemotron + the child context-budget lever — ONE changed condition vs row 64** | `docs/live-testing/briefs/arm-large-surface-requested.md`, `colleague/efforttables.py` (`purpose_context_override`), `scripts/compare_arms.py` | ✅ | PRE-REGISTERED 2026-08-30, BEFORE any run. Everything as row 64 (brief verbatim, no overlay, digest clause `b7491476…`, `offered_tools` 20 purpose-only, associate armed `COLLEAGUE_ASSOCIATE_MODEL=lobes`, every child must record `model: associate`, n=3, the six cells + compliance + post-digest read classification), except exactly ONE lever: `COLLEAGUE_CODE_SURVEY_CONTEXT_BUDGET=65536` (the #461 16–64K band top) caps each `code_survey` child's window; tip = the branch carrying `purpose_context_override` (v1.70.0). **Question:** does capping the survey child's window cut the child token burn (rows 63/64: 300–846k/child) and the wall (row 64 mean 1,742.9 s) without losing digest usability (ranged post-digest reads, task `ok`, planted pairs found)? **Pass bar (pre-stated):** mean wall < row 64's 1,742.9 s AND child mean total tokens < half of row 64's per-child mean (≈540k → bar < 270k) AND ranged-verify still 3/3 AND task 3/3 `ok`; promotion to a `PURPOSE_CONTEXT` table default ONLY if all four hold; a miss on any is written as a miss. Baselines frozen: row 64 artifacts `d5c57e32a644`,`a3e4b1726bf9`,`a56de2c569e6`; comparator sha `f7e25fdc…` unchanged. Refs: #458, #459, #461, PR #464. **Result: PENDING.** | #458 |
| 64c | delegation-follow-ups-a7-p3-hire (t20, decision c47): **arm R-digest — the requested-delegation brief re-run with the evidence-trail digest shape** | `docs/live-testing/briefs/arm-large-surface-requested.md`, `colleague/purpose_schemas.py`, `colleague/roles.py`, `scripts/compare_arms.py` | ✅ | PRE-REGISTERED before any run. Everything not stated here is as rows 63/64 (fixture, rig, comparator freeze, scope line, the six cells, the h16 artifact-id rule, the requested-delegation brief verbatim). **The ONLY lever:** the t20 digest shape at the tip under test — the two survey briefs demand path:start-end + a <= 5-line verbatim excerpt + a trailing 'commands run:' list, the scout fragment says the same, and the parent renderer prefixes an '[uncited digest: …]' line on a citation-free digest. **Question:** does a cited, excerpted digest turn the post-digest verification from full-module redo (row 63: 17/16/17 further steps, ALL eight duplicate-pair modules re-read in full, 0 ranged reads, 3/3 runs) into ranged verify-only reads of the cited lines? **Cells:** post-digest reads classified ranged-verify (`sed -n`/ranged `read_file`/`head`/`tail` on a cited range) vs full-module-redo, per run; uncited-marker count per run (expected 0 — a non-zero count is recorded, not hidden); compliance as row 63 (>= 4 `code_survey` before the first module-body read); wall vs row 63's 3,315/3,009/4,019 s and the pooled in-seat median 392 s. **Decision rule (pre-stated):** a majority of post-digest module reads ranged in >= 2/3 valid runs = the digest shape did its job; full redo again = negative, recorded as such. n=3. **Validity:** the parent `prompt_digest` is re-derived at registration time (the t20 change touches the CHILD brief and CHILD system fragment, not the parent system prompt — expected unchanged at `b7491476…`; if it moved, record the new digest and say why); `offered_tools` = the purpose-only surface, no raw name. **Seat:** as row 64's setup — `COLLEAGUE_ASSOCIATE_MODEL=lobes` once the #460 window fix lands and the operator confirms the Orin lane (row 64 d5); until then a cortex-children control run is labeled as such, never passed off as the associate arm. **Result: RECORDED 2026-08-31 — the decision rule PASSES: majority-ranged post-digest reads in 3/3 valid runs.** Artifacts `d19a89765eab` (run 1, tip `1d0b3bd`, 23:55:07-00:26:52), `474e744bf301` (run 2, tip `90dce51`, 00:26:52-00:51:34), `69cde59b032c` (run 3, tip `90dce51`, 00:51:34-01:15:42) - the mid-arm tip movement is DOCS-ONLY (`git diff 1d0b3bd..90dce51 -- colleague scripts tests` is empty: the three changed files are live-testing.md, hire-colleague.md, CLAUDE.md), so all three runs executed identical code. **Validity 3/3:** `prompt_digest` unchanged at `b7491476…` (as pre-registered - the t20 change is child-side), `offered_tools` the 20 purpose-only names; every child `model: associate` (12/12), 0 refusals; associate armed as row 64. **Compliance 3/3:** four `code_survey` at steps 1-4 before any module read. **Post-digest reads (the question):** 10/11, 14/14, 2/3 ranged - 26/28 ranged, 0 full reads of an UNEDITED module (the two full reads are the module under edit); run 3's parent verified with only THREE reads. vs row 63's 0-ranged / 3x-full-redo baseline: the digest shape did its job. **Uncited markers: 10/12 - recorded, not hidden, and mostly a FORMAT false-negative:** the digests are substantively cited (per-function line ranges in markdown tables, '(lines 79-1054)' with en-dashes), but only 2/12 used the pinned `path:start-end` colon form and `_CITATION_RE` accepted only colon/URL forms. Fixed AFTER the row in the same PR: the regex now also accepts `lines N` and en-dash numeric ranges; the brief still demands the colon form. **Wall:** 1,894 / 1,472 / 1,437 s (mean 1,601 - vs row 64's 1,742.9 and row 63's 3,447.5; still 4.1x the 392 s in-seat median). **Turns:** 14/9/9. **Task 3/3 `ok`** (`mod_a`, `mod_i`, `mod_i`), markup 0. **Children:** 10 ok / 2 incomplete at 309-958k tokens (no context cap in this arm - the digest shape moves the PARENT's behaviour, not the child burn). **Reading:** the evidence-trail digest shape is the first lever that changes what the parent DOES with a digest on this brief - ranged verification held 3/3 with the fewest post-digest reads of any arm - while delegation stays slower than in-seat (c46 stands). Next per #459: score digest CONTENT against fixture ground truth before any default changes. | #459 |
| 65 | delegation-follow-ups-a7-p3-hire (spec 2026-08-30, plan t16): **arm H — the HIRE knob on the repeated-sub-tasks brief, H vs control** | `docs/live-testing/briefs/arm-repeated-subtasks.md`, `scripts/make_repeated_subtasks_fixture.py`, `colleague/hire_schemas.py`, `colleague/hire_dispatch.py`, `colleague/hire_assign.py`, `scripts/compare_arms.py` | ✅ | PRE-REGISTERED 2026-08-30, BEFORE any run, queued behind rows 62-64. Arc: `delegation-follow-ups-a7-p3-hire` (spec `docs/specs/2026-08-30-delegation-follow-ups-a7-p3-hire.md`, plan t16, covers c21/h11). **Hypothesis (the input is row 61's verdict):** P3 — size-conditional delegation prose — went 0/3 against a clean P2-0 control and did not promote, so per-call prose is not the lever; this arm tests INSTEAD whether a persistent, purpose-agreed employee (one `hire_colleague` negotiation amortising across >= 6 similar assignments) gets chosen when the task shape repeats, vs one-shot purpose calls that amortise nothing. **Conditions:** arm H = `COLLEAGUE_HIRE=1`; control = knob unset (one-shot purpose tools only). Same brief verbatim (`docs/live-testing/briefs/arm-repeated-subtasks.md` — it never names a hire tool), same fixture rebuilt fresh per run, n=3 each, interleaved, sequential on the rig. **Tip pin:** every run executes on a tip carrying t10-t13's hire lane (`colleague/hire_schemas.py`, `hire_dispatch.py`, `hire_assign.py`) and t15's `hires`/`assignments` comparator columns; the exact SHA of each run is recorded beside its artifact id; an earlier tip VOIDS the run. **Fixture:** `scripts/make_repeated_subtasks_fixture.py` (deterministic; `tests/test_repeated_subtasks_fixture.py` pins two runs → identical trees): eight packages `pkgs/pkg_a`…`pkg_h`, each `core.py` 641 lines / 24,799 chars (one `read_file` page), total 5,128 lines / 198,392 chars, one seeded docstring/behaviour contradiction per package at a name-derived index; an `.eidetic` store as rows 57-64; the generator's printed answer key is operator-only and never committed into the fixture repo. **Rig:** recorded at run time in this row exactly as rows 59-64 did (gateway, cortex model, consumed roles, effort, `COLLEAGUE_TIMEOUT`, `max_steps`); a gateway stall / `step-stall` run is a rig failure: VOIDED and re-run (q4 precedent). **Comparator (h3):** `scripts/compare_arms.py` is not modified for this row beyond its merged t15 columns — `git diff` against the run tip recorded EMPTY at every run's SHA; ratio cells use `--bar-wall 1.2 --bar-turns 1.0` with the control as the baseline arm. **Validity (h18) — digest + surface, pasted per run:** each artifact carries `prompt_digest` and `offered_tools`. Control: all three control artifacts must share one digest (recorded from control run 1). **Arm H's digest is DERIVED, not free:** `tests/test_hire_schemas.py` pins `prompttext.default_system(hire=True) == V1_DEFAULT_SYSTEM + "\n\n" + SECTION_TABLE["HIRE"]` — the armed and unarmed composed prompts differ by exactly the ONE `HIRE` sentence (single period, no newline) — and `colleague/loop.py` computes `prompt_digest = prompt_digest_for(system_prompt)`; so the expected H digest is recomputed from the control's composed prompt text plus `"\n\n"` plus `prompttext.SECTION_TABLE["HIRE"]`, and every H artifact must match it; any other preimage difference VOIDS the pair. `offered_tools`: H runs end with `hire_colleague`, `assign_to_colleague` appended after the purpose splice (t11); control runs carry neither name; a mismatch VOIDS the run. **Pass bar (committed before the run, numeric):** the hire lane is USED iff hires >= 1 AND assignments >= 6 on >= 2 of 3 H runs — hires = `len(artifact['hires'])`, assignments = `assign_to_colleague` step count (t15's columns), never from prose. **A 0/3 result (no H run hires) is DECLARED PUBLISHABLE:** it is the finding, written as a miss — not a voided row. Control runs must show hires = 0 and assignments = 0 (schemas hidden unarmed); a nonzero control voids that run. **Cells to fill after the run (in this order, never before):** (1) hires per run; (2) assignments per run; (3) **accept/amend/decline counts per hire** — read off each `hire_colleague` step's result text (`hired: … (1 negotiation completion(s))` = round-1 accept; `… (2 …)` = round-2 accept with amended terms quoted beside it; `not hired: <reason>` = decline/refusal), pasted verbatim; (4) delegation histogram over `Step.tool` (each purpose tool and `assign_to_colleague` separately); (5) task success — status, all eight packages audited, contradictions named vs the generator's answer key (n/8 correct), exactly one docstring fixed; (6) turns ratio and wall ratio vs the control (`compare_arms.py --bar-wall 1.2 --bar-turns 1.0`); beside them markup count per run (`stats.counts.markup_tool_calls`; a zero-hire run with markup > 0 is INCONCLUSIVE), reasoning chars, `stats.counts` in full, stalls per run, each assignment child's served model + per-child `usage` from `sub_results[]`, the memory distill counters, and the artifact ids with their SHAs. **Every number comes from an artifact; a figure without an artifact id beside it is a defect in the row (h16).** **Audience (h12):** the operator reads this row and takes the one decision; cortex is OFFERED the hire surface and calls or ignores it explicitly — no cell records the runtime choosing on its behalf. **Honest caveats, carried from the brief:** the in-seat baseline CAN fit this fixture (~49,600 tokens, eight one-page reads), so an in-seat sweep is a rational choice and is recorded as such (c46); amortisation lives in the parent's authored context and agreed purpose, never in child warm-up (each assignment spawns a fresh run-scoped child); n=3. **Result: RECORDED 2026-08-31 — the hire lane was NOT used: hires 0/3, assignments 0/3 on every H run. Publishable as pre-declared; written as the finding, not a failure.** Runs interleaved on tip `349669c` (after one VOIDED H attempt: runner9's `[ \"$ARM\" = H ]` never matched the label `H65`, so its first H run started `hire=unset` — killed at 6 min, logged in runner.log, artifact discarded, runner9b fixed the match; every counted H run logs `hire=1`). Artifacts (order run): C `afc8687bdd85` (354 s, 9 turns) / H `fa54fae93a7a` (521 s, 10) / C `414c5c86a8cb` (693 s, 11) / H `ca84bab55170` (387 s, 11) / C `7bfb680cd928` (372 s, 13) / H `196bd9c40607` (678 s, 10). **Validity 6/6:** control digest `b7491476…` ×3 with NO hire name offered; H digest `b348c6ff…` ×3 with `hire_colleague`,`assign_to_colleague` appended — and the derived-digest clause PROVEN through the real composition path (a mock run unarmed/armed reproduces exactly that pair; relation = control prompt + `\n\n` + `SECTION_TABLE[\"HIRE\"]`). **Task success 6/6 `ok`, 8/8 contradictions vs the generator's answer key in ALL SIX runs** (scored off the run-branch audit files — H65-2's summary named only the fix; the branch file named all eight), exactly one docstring fixed each (pkg_a_step_02 in 5, correct per key). **Comparator (t15 columns):** control mean 473.2 s / 11.00 turns; H mean 528.4 s / 10.33 turns → wall_ratio 1.117 (pass), turns_ratio 0.939 (pass); hires/assignments columns 0/0 on every run; delegations 0 everywhere (no purpose call either — every run swept in-seat); markup 0. **Accept/amend/decline: n/a — no negotiation ever started.** **Instrument caveat found in review AFTER the runs (Qodo #469/2, fixed in the same PR):** `hires_block` was never called by the run path, so `TaskResult.hires` was structurally 0 on every artifact — the `hires` column could not have shown a hire even if one had happened. **The row's zero survives on independent evidence:** the STEP trace of all three H runs contains zero `hire_colleague` and zero `assign_to_colleague` steps (tools used: list_dir/read_file/grep_search/glob/write_file/edit_file/run_command/finish), so no negotiation was ever attempted; the `hires`/`assignments` columns are re-derivable from the step trace and agree. A future hire row reads the fixed artifact block directly. **Reading:** on a brief the seat can hold (~49.6k tokens, eight one-page reads), cortex ignores BOTH delegation forms — the persistent employee exactly as much as the one-shot purpose tools (rows 51–58's pattern extends to the hire surface; c46 stands). The armed surface costs nothing measurable (H inside both bars). The amortisation hypothesis is UNTESTED above the in-seat capacity ceiling — a brief too large to hold in-seat with >= 6 repeats does not exist yet; parked as the follow-up shape, mirroring the small-brief floor of rows 51-58. The one decision this row supports: `COLLEAGUE_HIRE` stays default-OFF. | #457 |

| 66 | effort-v4-rung-observability-rerank (plan t11, c21/h8): **the #475 rerun at `low` — INTERRUPTED, partial evidence only** | `colleague/effort.py` (v4 tables), `colleague/reasoninglog.py`, `colleague/continuation.py` | ✅ | The identical `6daa8d083e7b` brief (reduce loop.py 962 → <900, six source-text pins) dispatched against the identical repo state (worktree at `5a721b8f`, loop.py 962 lines) under the v4 harness (cortex `low`, rung recording + sidecar live). **Both segments were externally SIGTERM'd** (~2 min each; rig healthy, `/v1/models` 200): segment 1 artifact `2975aed37fb9` (2 steps, 2 turns, **reasoning 103 chars**), segment 2 `14a4df780b45` (`continued_from: 2975aed37fb9`, 7 steps, 6 turns, **reasoning 705 chars**). **The completion question (inside the 1800 s bound, six pins, suite green) is UNANSWERED — this row is not the A/B.** What the segments DO evidence: (1) the reasoning sidecar wrote per-turn `{seat, turn, request_ts, request_index, text}` records on a live run — the #476 observability lane works in production, and the rung/sidecar came from the run's own record with zero source archaeology; (2) reasoning volume at `low` is directionally tiny vs the medium arm — 705 chars over 6 turns vs 133,637 over 7 (~0.5%), the same early-turn procedure (read loop.py, locate pins); (3) `--continue` resumed the SIGTERM'd run cleanly (t8's lane). Recorded arms for the eventual completion A/B stay: medium 2,851 s incomplete / Claude control 506 s complete. Rerun when the operator frees the rig: `git worktree add <w> 5a721b8f` + the preserved brief verbatim + `work --continue 14a4df780b45`. | #475 |
| 67 | reasoning-aware-sampling-defaults-repetition-guard (plan t12, #479): **the sampling arm on a RECONSTRUCTED brief — criteria 1/2/4 PASS, criterion 3 FAIL** | `colleague/sampling.py`, `colleague/samplingwire.py`, `colleague/repetitionguard.py`, `colleague/engines/vllm_payload.py` | ✅ | **The brief is RECONSTRUCTED, not preserved — read every comparison below with that caveat.** colleague stores only `prompt_digest`, never the task text, so the `6daa8d083e7b` brief is unrecoverable; issue #475 describes it but does not list its six named source-text pins. I recovered the pins from the incident run's own FIRST action — a single grep for `read_control\(\)|pilot guidance|build_split_record|should_record_split|ThreadPoolExecutor|import asyncio` — and verified each against a real assertion at `5a721b8f` (`test_senses_live_presence_proofs.py:279/281`, `test_memory_split_record.py`, `test_toolbatch_loop.py:273`, `test_agents_ledger.py:527`). The prose is mine. Prose shape has moved outcomes by more than this arc's effect size in rows 51–58, so **the wall-clock figures here are NOT a like-for-like A/B against the recorded arms** (medium+greedy 2,851 s incomplete; Claude control 506 s complete) and must not be read as one. **Setup:** worktree at `5a721b8f` (loop.py 962 lines), cortex rung `low`, lobes `localhost:8001`, served `unsloth/Qwen3.8-27B-NVFP4`; `config show` before dispatch: `matched thinking row -> on the wire {'temperature': 1.0, 'top_p': 0.95, 'top_k': 20}; dropped as already the server default {'min_p': 0.0, 'presence_penalty': 0.0, 'repetition_penalty': 1.0}`. Artifact `cc5d1f1a2c5f`, its `sampling` block confirming the same row/wire pair on the run itself. **Criterion 1 PASS** — armed vs off-rung bodies differ in EXACTLY `temperature` (1.0→0.7), `top_p` (0.95→0.8), `presence_penalty` (absent→1.5) and `chat_template_kwargs` (`{reasoning_effort: low}`→`{enable_thinking: false}`); every other key identical, `top_k` 20 in both. Asserted as an empty set difference, not eyeballed. **Criterion 2 PASS (sensitivity)** — three completions at temperature 2.0 WITH `top_k: 1` byte-identical (`b66a7e837e` ×3), the same three WITHOUT it distinct (`474dc321e0`/`6d3db3f541`/`973840bc8a`). **The first form of this probe was falsified and is recorded as such:** prompt `Name one colour.` at `max_tokens 24` returned `'Blue'` ×3 in BOTH arms — identical everywhere, detecting nothing. Re-run open-ended at `max_tokens 120`, the arms separate. A discriminator that cannot fail is worth nothing. **Criterion 4 PASS** — the guard alone, sampling irrelevant (pure function, no adapter, no model): first detection at 704 emitted chars, cut after 5 detections at 1,056 chars, against the incident's 271,486 to `finish_reason=length` — a 257× reduction. Constants read off the module at run time (48/8/3). **Criterion 3 FAIL.** `status: incomplete` at the 40-step budget (step 60/40, forced synthesis), 1,984 s, 41 turns, 112,883 cumulative reasoning chars, 2 files changed. The extraction is structurally right — the per-tool-call lifecycle moved to a new `colleague/loop_tools.py` (212 lines), `loop.py` 962→820, and ALL FIVE statically-checkable pins hold (`read_control()` ×1, the `[pilot guidance]` append ×1, zero `build_split_record`/`should_record_split`, no top-level `import asyncio|socket`) — **but the delivered code does not import.** Two independent defects: (1) `from colleague.hooks import HookConfig, HookDecision, Policy` — no `Policy` exists in that module (it is a lowercase `policy` parameter typed `Any`), a hallucinated symbol; (2) with that removed, `ToolCall` is no longer re-exported from `colleague.loop`, breaking `colleague/engines/vllm_transport.py:38` — despite the run's report claiming a re-export block preserved the moved names. Not one line from green. Verified by checking the result branch out into a fresh worktree and running the suite, NOT by reading the run's own summary. **What DID hold, and matters:** longest single turn **18,407 chars** against the 50,000 bar (top five 18,407/16,276/10,672/7,264/6,391), and **zero repetition-guard trips** — against the incident's 271,486 characters on ONE turn with zero files changed. The reasoning-shape half of criterion 3 passed; the completion half did not. **The run's own report was exemplary and is why this row is trustworthy:** it explicitly refused to claim done ("⚠️ This is UNVERIFIED — I was cut off before running pytest"), listed the four checks an integrator should run in order, and named the `run_hook` lazy-import as the most likely follow-up. It was graded 2 — honest reporting, non-working code. **Honest limits:** n=1; reconstructed brief; the failure mode observed (budget exhaustion plus two import defects) is not the failure mode #479 set out to fix (a verbatim spiral to `finish_reason=length`), so this arm does NOT show sampling failing to help — it shows a different, unrelated wall. A clean answer to "does the card profile stop the spiral" needs the preserved brief, which no longer exists. **Filed from this row:** #480 (the affected-tests gate DID run here and reported `failed` — verified by re-running it against the result branch — but a non-`_EXIT_FINISHED` outcome gets zero fix turns and the failing report reached neither `TaskResult.warnings` nor the operator, so the model said "UNVERIFIED, I did not run pytest" while the harness already knew the branch was broken), #481 (the artifact persists only `prompt_digest`, which is what made this row's brief RECONSTRUCTED), #482 (a millisecond-cost pre-finish importability check would have named both defects directly), #483 (the t10 liveness helper is still unwired). Refs: #479, #475, row 66, deviations d1–d6. | #479 |
| 68 | reasoning-aware-sampling-defaults-repetition-guard (plan t12, #479): **the step-budget arm — criterion 3's COMPLETION half PASSES, its reasoning-shape half FAILS** | `colleague/sampling.py`, `colleague/samplingwire.py`, `colleague/repetitionguard.py` | ✅ | **OPERATOR ERROR, CORRECTED — this row is NOT the medium arm it was dispatched as.** I set `COLLEAGUE_MAIN_EFFORT=medium`; that is **not a knob** (the real one is `COLLEAGUE_REASONING_EFFORT`), and the artifact records `effort: {"main": "low"}`. Four status ticks narrating a "medium signature" were narration over a variable that never changed. Arm 2 ran at **`low`**, identical to row 67's arm 1. **The only real differences were `COLLEAGUE_MAX_STEPS=90` (vs 40) and `--until-done --max-episodes 3`, which was never exercised — the run finished inside ONE episode at 63 steps.** Everything below is therefore a step-budget comparison at a FIXED rung and a FIXED sampling profile, not a rung comparison. Same RECONSTRUCTED brief as row 67 (six pins recovered from the incident run's first grep; the prose is mine), same worktree base `5a721b8f` (loop.py 962 lines), same served `unsloth/Qwen3.8-27B-NVFP4`, same card profile on the wire (`temperature 1.0 / top_p 0.95 / top_k 20`, confirmed on the artifact's own `sampling` block). **RESULT: `status: ok`.** 63 steps, 47 turns, 5,244 s, 413,134 reasoning chars, 2 files changed. `loop.py` 962 → **881**, new `colleague/loop_toolchain.py` (225 lines). **Verified by me on the result branch, not read off the run's summary:** all six pins hold (`read_control()` ×1, the `[pilot guidance]` append ×1, zero `build_split_record`/`should_record_split`, no top-level `import asyncio|socket`, and `test_toolbatch_loop.py` green); `affected_tests_report` **passed**; `uv run pytest -n auto -q` → **11,226 passed, 51 skipped**; the six pin-guarding test files → 1,958 passed. The run also ran `isort`, `black`, `flake8`, `bandit` and `teken cli doctor --strict` itself. **Head to head with row 67 (arm 1), same rung, same sampling:** arm 1 at 40 steps went `incomplete` at step 60/40 and shipped a branch that **does not import** — a hallucinated `Policy` symbol from `colleague.hooks`, and a lost `ToolCall` re-export breaking `vllm_transport.py`; its affected-tests gate reported **failed**. Arm 2 at 90 steps avoided **both** defects by construction: its 40,014-char planning turn verified the import DAG (siblings never import `loop.py`) and verified `toolbatch_loop`'s `_loop.<name>` lookups still resolve via re-export, before writing a line. **The binding constraint in row 67 was the STEP BUDGET, not the rung and not sampling.** **Criterion 3 verdict: SPLIT.** Completion half **PASS** (finished inside its bound where arm 1 did not, suite green, pins intact). Reasoning-shape half **FAIL**: longest single turn **119,602 chars** against the 50,000 bar — 2.4× over, at `low`. Zero repetition-guard trips (nothing is verbatim; the model re-derives rather than repeats, which no guard in this arc addresses). **Where the reasoning went (the decide-vs-act finding, rung-independent).** Pairing every reasoning turn with the tool calls that followed it: every heavy turn sits in the read-only survey, and the acting tail collapses to hundreds of characters (`638`, `195`, `148`, `114`…). The single exception is the 119,602-char turn, which fused a DECISION (what goes in the new module) with an ACTION (writing it) and cost 3× the plan turn. Survey heavy turns 176,808 + the write turn 119,602 = **296,410 of 399,320 (74%) spent on turns that were acting, not deciding**. This is the measured basis for #484. **Honest limits:** n=1 per arm; reconstructed brief, so neither arm compares like-for-like to the recorded medium+greedy 2,851 s incomplete or the Claude control 506 s complete; the rung was never varied, so **this arc still has NO clean measurement of sampling's marginal effect over the #475 rung change** — rows 66, 67 and 68 all sit at `low`, and row 66 (the intended `low`+greedy control) was SIGTERM'd. A `COLLEAGUE_SAMPLING=0` run at 90 steps on this same brief is the missing control and is NOT recorded here because it was not run. Refs: #479, #475, #482, #484, row 66, row 67. | #479 |
| 69 | small-fixes-then-effort-balance (plan t12, #480-#483): **the row-67 failure shape REPRODUCED under the v1.75.0 harness — all four fixes fired; PASS** | `colleague/importcheck.py`, `colleague/tasktext.py`, `colleague/loop_deltaheartbeat.py`, `colleague/loop_testgates_warnings.py` | ✅ | Same RECONSTRUCTED-AGAIN brief (the cc5d1f1a2c5f worktree was reaped, so the six pins were re-rendered from row 67's recipe — the THIRD demonstration of #481's archaeology, and the last: this run records `task_text`), same base `5a721b8f` (loop.py 962), served `unsloth/Qwen3.8-27B-NVFP4`, cortex `low`, `COLLEAGUE_MAX_STEPS=40`, spikes OFF. **RESULT `status: incomplete`** at step 63/40 (forced synthesis), ~3,450 s, `loop.py` 962→885 + new `colleague/loop_prepare.py` — **and the branch again does not import**: `cannot import name '_build_initial_content' from 'colleague.loop_prepare'` (the run's loop.py re-export names a symbol the extracted module never defines — the same hallucinated-re-export class as row 67). The difference is the harness: **#482** `import-check-failed` warning + `importcheck_report: failed` on the artifact naming `colleague.loop` and the exact ImportError, on a budget-exhausted outcome; **#480** `affected-tests-failed` warning naming the failing selection (row 67's identical gate result reached no one); **#481** `task_text` carries the dispatch brief verbatim on the artifact — a rerun is dispatchable with zero operator memory; **#483** the flight feed held continuous mid-turn `receiving…` heartbeats across the whole 57-minute run (650+ records; the longest survey stretch produced 200+ liveness records where row 67 was silent by construction). `effort_spikes` absent = did-not-fire (spikes unarmed; correct reading). A `capacity_warning` (backpressure at 256 s avg turns toward the 300 s timeout) was also recorded. **Honest limits:** n=1; the brief is reconstructed prose (rows 51-58 caveat applies — no wall-clock A/B against rows 67/68); the model failing the same way twice is consistent with, not proof of, the 40-step budget being the binding constraint (row 68's 90-step arm avoided it). Artifact `39661f2af608` at the t12 target worktree. Refs: #480, #481, #482, #483, #484, rows 67-68, spec 2026-09-01-small-fixes-then-effort-balance. | #480 #481 #482 #483 |
| 70 | measure-effort-spikes-484 (spec 2026-09-01-measure-effort-spikes-484, c3/h3): **the armed barrier fires LIVE for the first time — pre-flight smoke PASS** | `colleague/effortspikes.py`, `colleague/loop_barrier.py` | ✅ | The #484 surface merged dark in v1.75.0 with unit proof only (row 69 ran spikes OFF), so before any measurement arm one smoke on a throwaway two-file repo (`tiny.py` with one `add()` function; brief: read it, then add `double(x)` after `add()`), `COLLEAGUE_EFFORT_SPIKES=1 COLLEAGUE_TIMEOUT=600 COLLEAGUE_MAX_STEPS=12`, served `unsloth/Qwen3.8-27B-NVFP4` via lobes at `localhost:8001`, cortex `low`. Artifact `2db5bb0ae410`: `status: ok`, `effort {main: low}`, **`effort_spikes: [{point: barrier.pre_mutation, rung: medium, seat: cortex}]`**, steps `read_file → barrier.pre_mutation → edit_file → read_file → finish` (5 steps, 6 model turns — the intercepted turn is counted, decision c23), warnings NONE (so no `effort-spike-barrier` degradation), the flight feed carried the `planning the change before the first edit` phase notice, and the trace holds the barrier's plan text (`**Plan** 1. File: tiny.py — append double(x) after add() …`). Reasoning sidecar per turn: 1 (read) short, 2 (the intercepted edit turn) 153 chars, **3 (the barrier, medium) 549 chars — the largest in the run**, 4 (the re-issued edit) 221, 5 (verify read) 46, 6 (finish) 94. The result branch carries exactly the requested four-line addition. **Honest limits:** trivial brief, n=1; nothing records the per-turn wire payload, so `reasoning_effort: medium` reaching the server rests on the seat attribute (`tests/test_barrier_pre_mutation.py:365`) and the ladder tests, plus the reasoning-volume shape above — not on a live capture; the reasoning sidecar labels the barrier turn `seat: cortex` with no rung field (a small observability gap worth a follow-up). This row is a mechanism proof, NOT an arm — the A/B/C arms follow in rows 71-73. Refs: #484, row 69. | #484 |
| 71 | measure-effort-spikes-484 (plan t2, arm A): **low + feedback, spikes OFF, 90 steps — `status: ok`, branch imports, all six pins hold, suite green; correctness PASS** | `colleague/importcheck.py`, `colleague/loop_testgates_warnings.py`, `colleague/effortspikes.py` (unarmed) | ✅ | **The arm the #482 comment pre-registered as the null hypothesis, now on a PRESERVED brief:** dispatched FRESH from row 69's artifact `39661f2af608` `task_text` (700 chars; the arm artifact's `task_text` sha256 `815f5c3f…1ac9` equals the brief file's — h2 asserted, not eyeballed), NOT via `--continue b702d8249dea` (that chain held zero progress and a seed-contaminated `task_text`; spec s4/s5). Env read off the artifact: `effort {main: low}`, sampling `[main, thinking, {temperature 1.0, top_p 0.95, top_k 20}]`, no `effort_spikes` key (unarmed = did-not-fire), `COLLEAGUE_MAX_STEPS=90`, `COLLEAGUE_TIMEOUT=600`; target worktree `sf-arms` at `5a721b8f` (loop.py 962), served `unsloth/Qwen3.8-27B-NVFP4`. **RESULT `status: ok`**, `not_finished: false` — the model called `finish` itself at step 94 after running black/isort, `wc -l`, `git diff --stat` and `check_test_integrity`; 95 steps / **77 model turns** (steps exceed 90 because parallel read-only batches count per call: 7 batches, 16 calls parallelised), **3,140 s** wall, **150,284 reasoning chars** total, longest single turn **19,973** (top five 19,973 / 16,794 / 14,153 / 8,462 / 7,261), usage prompt 4,259,910 / completion 56,497 tokens, 2 files changed (`loop.py` 962 → **817**, new `colleague/loop_toolchain.py` — the per-tool-call execution chain: `_fire_hooks`/`_gate_tool_call`/`_record_denial`/`_fire_post_tool`/`_record_execution`/`_run_tool_call`/`_run_tool_calls`, re-exported). Tool mix: run_command 51, edit_file 18, read_file 13, grep_search 8, write_file 2. **Feedback surfaces (the arm's variable):** `importcheck_report: passed` (checked `loop.py`, `loop_toolchain.py`), `affected_tests_report: passed` (20 files capped from 241, 250 passed), lint gate ran isort+black, warnings NONE — so **no fix-turn was ever needed**: the run FINISHED and the gates passed first time (h7: the fed-back fix turn exists only on a finished outcome, and here it had nothing to feed back). **Correctness verified by me on the result branch `colleague/26d71865aeee-…` checked out into a fresh worktree, not read off the summary:** `python -c 'import colleague.loop'` → `IMPORT OK`; pins (1) `read_control()` ×1, (2) `[pilot guidance]` ×1, (3) `build_split_record` 0, (4) `should_record_split` 0, (5) top-level `ThreadPoolExecutor` import 0, (6) top-level `import asyncio` 0; `uv run pytest -n auto -q` → **11,226 passed, 51 skipped**. **Head to head with row 68 (same brief shape, same rung, same sampling, 90 steps, spikes off):** row 68 took 47 turns / 5,244 s / 413,134 reasoning chars / longest turn 119,602; this arm 77 turns / 3,140 s / 150,284 chars / longest 19,973 — same correctness at 36% of the reasoning and 60% of the wall, with the decide-vs-act fusion turn absent (no turn above 20k). The only harness differences are v1.75.0's four fixes (none fired) and the preserved brief; **n=1 in both, so this is directional, not a measured effect of #480-#483.** **Honest limits:** n=1; the run never exercised the feedback path (nothing failed), so arm A shows "cheap feedback was not NEEDED here", not "cheap feedback fixes a broken branch"; `usage.prompt_tokens` is cumulative across 77 turns, not a context size. Artifact `26d71865aeee` at `sf-arms/.colleague/`. Refs: #484, #482, rows 67-70, spec 2026-09-01-measure-effort-spikes-484 (c2/h2/c7/h5). | #484 |
| 72 | measure-effort-spikes-484 (plan t3, arm B): **spikes ON with the barrier pinned `low` — the barrier NEVER FIRED, twice: arm B is VOID as a barrier arm; its rerun stands as a wire-identical replicate of arm A (correctness PASS)** | `colleague/loop_barrier.py` (`should_fire`), `colleague/roles.py:78` (`_WRITE_TOOLS`) | ✅ | Same preserved brief as row 71 (`task_text` sha256 `815f5c3f…1ac9` on both attempts' artifacts), same target `sf-arms` at `5a721b8f`, same model/rung/sampling, `COLLEAGUE_MAX_STEPS=90`, `COLLEAGUE_TIMEOUT=600`, plus `COLLEAGUE_EFFORT_SPIKES=1 COLLEAGUE_EFFORT_SPIKE_BARRIER_PRE_MUTATION=low COLLEAGUE_EFFORT_SPIKE_GATE_REPEAT_FAILURE=low COLLEAGUE_EFFORT_SPIKE_FILLLINE_DECISION=low` (q3: only the barrier rung was meant to vary between B and C). **What happened instead — the trigger, not the rung, decided the arm.** `loop_barrier.should_fire` fires only while EVERY prior step named a read-only tool, and `run_command` is a mutating tool BY NAME (`roles._WRITE_TOOLS`). **Attempt 1** (`4f362863a7b5`) opened with `run_command` (`git status; git diff --stat; wc -l …`) at step 0 — from that step on the barrier could never fire in that run; stopped cooperatively via `flight stop` at step 8 (`incomplete`, `pilot_stop`, 0 files) as the h4 VOID, and **rerun once. Attempt 2** (`d6088d427346`) ALSO opened with `run_command` (`wc -l colleague/loop.py; grep -n read_control …`) at step 0 — VOID again. Arm A (row 71) had opened with `read_file` and shelled out second, which is the only reason its trace could have hosted a barrier at all. **Three openings on this brief: read-first 1, shell-first 2.** No `effort_spikes` key on either artifact (= did-not-fire, the correct reading), no `effort-spike-barrier` warning (the seat was never even built for a turn), so this is not the timeout degradation c4 guarded against — it is the v0 trigger's own precondition failing on a model that surveys with the shell. **The rerun as a data point:** with every spike point either unfired or pinned to the seat's own `low`, attempt 2's outgoing payloads are wire-identical to arm A's, so it is an honest n=2 for arm A's condition. **RESULT `status: ok`**, `not_finished: false`, `finish` called at step 46; 47 steps / **39 model turns** (1 parallel batch, 2 calls), **2,877 s**, **146,083 reasoning chars**, longest turn **25,808** (top five 25,808 / 24,388 / 18,482 / 12,089 / 11,577), usage prompt 1,675,887 / completion 53,263, `loop.py` 962 → **784**, new `colleague/loop_toolchain.py` (241 lines; the same per-tool-call lifecycle chain arm A chose, plus `_tae_commit_initial_plan`), `importcheck_report: passed`, `affected_tests_report: passed` (250 passed), warnings NONE, no fix-turn needed. Tool mix: run_command 19, edit_file 14, read_file 9, write_file 2, run_tests 2. **Correctness verified by me on the result branch in a fresh worktree:** `import colleague.loop` → OK; pins (1) ×1, (2) ×1, (3) 0, (4) 0, (5) 0, (6) 0; `uv run pytest -n auto -q` → **11,226 passed, 51 skipped**. **Arm A vs its replicate (n=2, same condition):** turns 77 vs 39, wall 3,140 vs 2,877 s, reasoning 150,284 vs 146,083 chars, longest turn 19,973 vs 25,808 — reasoning volume within 3%, turn count 2× apart (arm A ran 51 shell commands to this run's 19). That spread is the noise floor any B/C spend comparison on this brief would have to clear (park r2). **Honest limits:** the arm answers nothing about the barrier's rung; it establishes that the shipped v0 trigger is unmeasurable on this brief without a trigger change (a code change outside this arc's c8/c9 boundary — filed as a follow-up from the #484 disposition, decision q6/c20); n=2 on one brief. Artifacts `4f362863a7b5` (void) and `d6088d427346` (replicate) at `sf-arms/.colleague/`. Refs: #484, rows 70-71, spec 2026-09-01-measure-effort-spikes-484 (c4/h4, s9-s11). | #484 |
| 73 | measure-effort-spikes-484 (plan t4, arm C): **spikes ON, barrier at the table's `medium` — the barrier FIRED at step 21 after a 20-step read-only survey, wrote the plan #484 imagined, and the run landed correct — at 2.3× arm A's reasoning and 1.5× its wall. Correctness PASS; C does NOT beat A (and B could not be measured)** | `colleague/loop_barrier.py`, `colleague/effortspikes.py` | ✅ | Same preserved brief (`task_text` sha256 `815f5c3f…1ac9`), target `sf-arms` at `5a721b8f`, model/rung/sampling as rows 71-72, `COLLEAGUE_MAX_STEPS=90`, `COLLEAGUE_TIMEOUT=600`, `COLLEAGUE_EFFORT_SPIKES=1` with gate/fillline pinned `low` (q3) and the barrier left at `SPIKE_TABLE`'s `medium`. **Two attempts before a measurable run:** attempt 1 (`1452182d4e80`) opened shell-first (`run_command wc -l; ls -la`) — VOID under c4, stopped via `flight stop` at step 9; the re-dispatch (`c46cea837568`) died with its launcher shell before step 0 (SIGTERM, not a model outcome — relaunched detached via `setsid`). **Five dispatches on this brief across B and C: shell-first 3, read-first 2** — the v0 trigger's coin flip, recorded on the frame (s9-s12). **The measurable run (`7af9b55d66a6`)** opened `read_file colleague/loop.py`, surveyed for 20 read-only steps (13 `read_file`, 12 `grep_search` across the run; 9 parallel batches, 19 calls parallelised), and on the turn that first requested `write_file` the barrier interposed: `effort_spikes: [{point: barrier.pre_mutation, rung: medium, seat: cortex}]`, a `barrier.pre_mutation` Step at **index 21** (`{rung: medium}`), the flight feed's `planning the change before the first edit` notice, warnings NONE (no `effort-spike-barrier` degradation — the seat built and the turn completed inside the 600 s timeout), and a **5,661-char plan** on the trace that names the new module, the seven functions to move verbatim, the isort-ordered import list, the `toolbatch_loop` lazy-import seam and the re-export block — i.e. the "verified the import DAG and the re-export seam" turn #484 quoted from row 68, now produced ON PURPOSE at a fixed point. **RESULT `status: ok`**, `not_finished: false`, `finish` at step 47; 48 steps / **29 model turns**, **4,581 s**, **341,791 reasoning chars**, longest turn **78,631 = the barrier turn itself** (top five 78,631 / 42,097 / 36,738 / 30,814 / 27,968; the write turn that followed the plan reasoned only 608 chars — the decide/act split #484's comment asked for, observed), usage prompt 1,378,008 / completion 109,442, `loop.py` 962 → **796**, new `colleague/loop_toolchain.py` (210 lines; the same lifecycle chain arms A and B chose), `importcheck_report: passed`, `affected_tests_report: passed` (250 passed), a `capacity_warning` (backpressure at 333 s average turns toward the 600 s timeout). **Correctness verified by me on the result branch in a fresh worktree:** `import colleague.loop` → OK; pins (1) ×1, (2) ×1, (3) 0, (4) 0, (5) 0, (6) 0; `uv run pytest -n auto -q` → **11,226 passed, 51 skipped**. **The comparison (q4 pre-stated: at equal correctness, lower total spend wins):** | arm | turns | wall s | reasoning chars | longest turn | correctness | A (row 71) 77 / 3,140 / 150,284 / 19,973 / PASS · A-replicate (row 72) 39 / 2,877 / 146,083 / 25,808 / PASS · **C** 29 / **4,581** / **341,791** / 78,631 / PASS · B (barrier `low`) VOID ×2, no measurement. Equal correctness across every measurable arm; C spends **2.3× the reasoning and 1.5× the wall** of the two A runs (which agree with each other within 3% on reasoning), buying fewer turns (29 vs 39/77) and nothing on the primary measure. **The pre-registered C-beats-B test could not be run** (B void); by the q4 tie-break C loses to A outright, and A itself never needed its feedback path (rows 71-72: no gate ever failed). **Reading, in #482's pre-registered wording: "same correctness at materially lower reasoning spend → this issue [#482, with #480] is the fix; close #484"** — with the caveats that the spend gap is n=2 vs n=1 on one brief, that the rung-`low` barrier (arm B) is unmeasured, and that the v0 trigger fired on only 2 of 5 dispatches, so a `low`-barrier or a reliably-firing barrier remains untested rather than refuted. **Honest limits:** n=1 for C; the barrier turn's 78k chars at `medium` is itself 52% of arm A's entire run; the fill-line never crossed (park r2's second half — `fillline.decision` stays unmeasured, no entry on any artifact); nothing captures the per-turn wire payload, so `medium` on the wire for the barrier rests on the seat attribute + unit tests + the 78k reasoning shape; the four VOID/killed dispatches cost ~1.5 h of rig time that the trigger follow-up would have saved. Artifacts `7af9b55d66a6` (measured), `1452182d4e80` (void), `c46cea837568` (killed) at `sf-arms/.colleague/`. Refs: #484, #482, #480, rows 67-72, spec 2026-09-01-measure-effort-spikes-484 (c4/h4/c7/h5/c14). | #484 |
| 66b | reasoning-aware-sampling-defaults-repetition-guard (#479, plan t11): **the greedy-at-`low` incident read off the artifacts, and the four live probes with their limits — EVIDENCE, not an arm** | `colleague/sampling.py`, `colleague/samplingwire.py`, `colleague/samplingfile.py`, `colleague/repetitionguard.py`, [`docs/features/sampling.md`](features/sampling.md) | ✅ | **Not a measurement arm and pre-registers nothing** — it records what is already on disk so the #479 claims are checkable rather than quoted. **(1) The incident.** Run `2bd306a6916a` (the row 66 `low` segment's continuation, greedy `temperature 0.0` with thinking armed; artifact + reasoning sidecar under `.colleague/2bd306a6916a.*` in the `rerun-low-arm` worktree): `status: error`, 31 steps, 22 model turns, **651,679 reasoning chars total**, and ONE warning — `{kind: truncated-turn, finish_reason: length, reasoning_chars: 271486, step_index: 20}`. The per-turn sidecar profile is 1: 320, 2: 227, 3: 34, 4: 34, **5: 46,998**, 6: 364, 7: 151, 8: 21,997, 9: 35,149, 10: 60,747, 11: 99,658, 12: 1,326, 13: 78,049, **14: 271,486**, 15: 2,811, 16: 50, 17: 236, 18: 76, 19: 9,769, 20: 7,087, 21: 14,936, 22: 174. **Correction recorded, not smoothed:** the spec's paraphrase says turns 1-11 reasoned sharply; the sidecar says the escalation starts at turn 5 and turn 11 is already 99,658 chars. The post-retry recovery half of the claim DOES hold (turns 16-18 and 22 are 50-236 chars, with 19-21 at 7k-15k). Run `4b74a1bd5a9b` (artifact in this repo's `.colleague/`) is the second half: `status: incomplete`, one warning `{kind: loop-guard, guard: identical-calls, tool: write_file, repeats: 5, limit: 5, dropped: 1}`. **(2) The four probes, 2026-09-01, against the operator's lobes gateway at `localhost:8001` serving Qwen3.8-27B-NVFP4** — recorded observations, NOT tests; nothing in the suite re-runs them: extension keys are forwarded and honoured (`top_k: 1` at temperature 2.0 → three byte-identical completions; without `top_k` → three different, one carrying the garbled token "Reykitanjör") — limit: that rig, that moment; **a 200 proves nothing** (the invented key `colleague_bogus_key` also returned 200, so no test may treat a status code as proof — which is why probe 1 is a determinism experiment) — limit: another server may 400 instead; `presence_penalty 1.5` is harmless to tool calling (3/3 well-formed parallel calls, same as the 0.0 control) and to bulk code emission (154 lines vs a 158-line control, ZERO non-ASCII — no language mixing) — limit: one model, one rig, small sample, structural metrics only, correctness unevaluated; aborting the SSE read DOES stop generation (GPU 95% → 3% in ~6 s) — limit: observed via GPU utilisation, not a server-side confirmation, and utilisation was already 95% from a prior killed request. **(3) A reproduced guard false POSITIVE:** an 84-character boilerplate narration line repeated 8x back-to-back trips `repetitionguard`; 7x does not, and interleaving varying text does not. Found by an `ask-colleague` review (task `4754361a0229`), reproduced independently, and re-reproduced on every suite run by `tests/test_sampling_docs.py`. The bounded cost (one TURN into the existing retry, never the run, until the 3rd trip) is why the arc records the limit instead of weakening the detector. False NEGATIVE: the tail-only 8192-char window misses a runaway that recovers inside the last 8 KB. **The definitive sampling arm — the preserved brief rerun at the full Qwen3.8 thinking profile — is a SEPARATE task and is NOT in this row.** Until it lands, nothing here claims the sampling change made a run complete that greedy decoding did not. | #479 |
